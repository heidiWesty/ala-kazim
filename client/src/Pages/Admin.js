...
function Admin() {
  ...
  const getDataForAbscencesChart = () => {
    ...
    chartData.forEach((data, index) => {
      chartData[index].value = getAbsencesForMonth(data.month)
    });
  ...
  const getAbsencesForMonth = (requestedMonth) => {
    ...
    let absencesForRequestedMonth = data.attendance.filter(attendanceRecord => {
      let currentYear = new Date().getFullYear();
      let recordYear = attendanceRecord.date.getFullYear();
      let recordMonth = attendanceRecord.date.getMonth() + 1;
      return requestedMonth === recordMonth && currentYear === recordYear && !attendanceRecord.wasPresent && attendanceRecord.classId === currClass.id;
    });
  ...
  const getDataForPresentChart = () => {
    ...
    chartData.forEach((data, index) => {
      chartData[index].value = getAttendanceForMonth(data.month)
    });
  ...
  const getTotalStudentsForCurrClass = (data, currClass) => {
    ...
    data.attendance.forEach(record => {
      if (!uniqueStudents.includes(record.studentId) && record.classId === currClass.id) {
    ...
    <Grid container spacing={2} style={{ padding: '10px' }}>
      <Grid item xs={2}>
        <AppBar position="static">
          <Tabs orientation="vertical" value={tabValue} onChange={handleSetTabValue}>
            {
              data &&
              data.classes.map(c => <Tab key={Math.random()} label={c.name} />)
            }
          </Tabs>
          <Tab onClick={() => history.push("camview")} label={camView}>
      ...
      <Grid container spacing={1} xs={10} style={{ padding: '17px' }}>
        <Grid xs={12} align="center">
          <Typography variant="h3">Welcome, professor!</Typography>
        </Grid>
        <Grid xs={3} item align="center">
          <Card style={{ padding: "10px", minHeight: "100px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <Typography variant="h4">{currClass != null ? currClass.name : "Loading..."}</Typography>
          </Card>
        </Grid>
        <Grid xs={3} item>
          <Card style={{ padding: "10px", minHeight: "100px" }}>
            ...
            <Grid xs={12}>
              <Typography variant="h4">{getTotalStudentsForCurrClass(data, currClass)}</Typography>
            </Grid>
          </Card>
        </Grid>
        <Grid xs={3} item align="center">
          <Card style={{ padding: "10px", minHeight: "100px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <Button variant="contained" onClick={addAttendance}>Add Attendance</Button>
          </Card>
        </Grid>
        <Grid xs={3} item align="center">
          <Card style={{ padding: "10px", minHeight: "100px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <Button variant="contained" onClick={editAttendance}>Edit Attendance</Button>
          </Card>
        </Grid>
        <Grid xs={6} item>
          <Card>
